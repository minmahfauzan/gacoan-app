...
<body class="min-h-screen flex">
    ...
    <aside id="sidebar" class="sidebar expanded bg-white shadow-md h-screen sticky top-0 overflow-y-auto">
        ...
        <nav class="py-4">
            <a href="{{ route('admin.dashboard') }}" class="nav-link flex items-center px-4 py-3 text-gray-700 hover:text-red-600 {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.orders.index') }}" class="nav-link flex items-center px-4 py-3 text-gray-700 hover:text-red-600 {{ request()->routeIs('admin.orders.*') ? 'active' : '' }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="absolute bottom-0 left-0 right-0 p-4 border-t">
            <form action="{{ route('admin.logout') }}" method="POST">
                @csrf
                <button type="submit" class="nav-link flex items-center w-full px-4 py-3 text-gray-700 hover:text-red-600">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
